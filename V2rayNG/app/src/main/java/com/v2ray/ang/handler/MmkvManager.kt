...
//        profileStorage.encode(key, JsonUtil.toJson(profile))
    ...
    /**
     * Removes all server-related data while keeping user settings.
     *
     * SECURITY:
     * - Used by locked commercial build to ensure only API servers exist.
     */
    fun removeAllServersKeepSettings(): Int {
        val count = profileFullStorage.allKeys()?.count() ?: 0
        // Remove server list + selected server only (do NOT touch settingsStorage).
        mainStorage.remove(KEY_SELECTED_SERVER)
    ...
    /**
     * Clears all subscriptions and their indices.
     *
     * SECURITY:
     * - Locked build should not keep user-provided subscription sources.
     */
    fun clearAllSubscriptions() {
        // Remove subscription index first
        mainStorage.remove(KEY_SUB_IDS)
        // Remove all subscription items
        subStorage.allKeys()?.forEach { key ->
