...
import com.v2ray.ang.api.ApiServersImporter
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    sealed class ApiSyncState {
        data object Idle : ApiSyncState()
        data object Syncing : ApiSyncState()
        data object Synced : ApiSyncState()
        data class Failed(val reason: String? = null) : ApiSyncState()
    }
    ...
    /**
     * Locked build: refresh server list from API on app start.
     *
     * SECURITY:
     * - If API fails, wipe servers (higher security).
     */
    fun syncServersFromApiOnStart() {
        apiSyncState.value = ApiSyncState.Syncing
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                // On error: wipe servers to avoid fallback to non-API data
                ApiServersImporter.replaceAll(emptyList())
                withContext(Dispatchers.Main) {
                    apiSyncState.value = ApiSyncState.Failed(e.message)
